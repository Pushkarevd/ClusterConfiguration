...
class Status:
    ...
    def __wait_connection(self):
        for tries in range(1, 11):
            try:
                ...
                LOGGER.warning(
                    f"Can't connect to {self._ip}:{self._port}\n" f"Attempt {tries}"
                )
            if tries == 10:
    ...
    def __get_init_msg(self):
        ...
        LOGGER.info(
            f"Ventilator port - {self._ventilator_port}\n"
    ...
    def __established_connection(self):
        ...
        self._status_thread = threading.Thread(
            target=self.__status_loop, daemon=True, name="status_thread"
        )
    ...
    def __status_loop(self):
        while True:
            ...
            if msg.decode() != "STATUS":
                raise ConnectionError(
                    f"Command from server is wrong,"
